...
├── admin_bot/              # Модуль админ-бота
├── admin_handlers/         # Обработчики сообщений для админ-бота
├── utils/                  # Утилиты
│   ├── db/                 # Модуль для работы с базой данных
│   │   ├── __init__.py     # DatabaseManager
│   │   ├── database.py     # Низкоуровневое управление БД
│   │   ├── user_queries.py # Запросы к таблице users
│   │   └── ...             # Другие файлы с запросами
...
- **Рефакторинг управления базой данных.**
  - Модуль `utils/db.py` был разделен на пакет `utils/db/`.
  - `DatabaseManager` теперь централизует доступ к базе данных и объединяет классы запросов (`UserQueries`, `BookingQueries`, и т.д.).
  - Это улучшает структуру, упрощает тестирование и делает код более модульным.
- Исправлены ошибки импорта и инициализации, связанные с рефакторингом.
